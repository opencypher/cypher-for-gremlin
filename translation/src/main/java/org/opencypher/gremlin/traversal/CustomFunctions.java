...
public class CustomFunctions {
    ...
    public static Function<Traverser, Object> length() {
        return traverser -> (((Path) traverser.get()).size() - 1) / 2;
    }

    public static Function<Traverser, Object> convertToString() {
        return traverser -> Optional.ofNullable(traverser.get())
            .map(String::valueOf)
            .orElse(null);
    }

    public static Function<Traverser, Object> convertToBoolean() {
        return traverser -> {
    ...
    public static Function<Traverser, Object> convertToInteger() {
        return traverser -> {
            Object arg = tokenToNull(traverser.get());
            boolean valid = arg == null ||
    ...
    public static Function<Traverser, Object> convertToFloat() {
        return traverser -> {
            Object arg = tokenToNull(traverser.get());
            boolean valid = arg == null ||
                arg instanceof Number ||
    ...
    public static Function<Traverser, Object> nodes() {
        return traverser -> ((Path) traverser.get()).objects().stream()
            .filter(element -> element instanceof Vertex)
            .collect(toList());
    }

    public static Function<Traverser, Object> relationships() {
        return traverser -> ((Collection) ((Path) traverser.get()).objects()).stream()
            .flatMap(CustomFunctions::flatten)
            .filter(element -> element instanceof Edge)
            .collect(toList());
    }

    public static Function<Traverser, Object> listComprehension(final Object functionTraversal) {
        return traverser -> {
    ...
    public static Function<Traverser, Object> pathComprehension() {
        return arg -> ((Collection) arg.get()).stream()
    ...
    public static Function<Traverser, Object> containerIndex(Object index) {
        return traverser -> {
    ...
    public static Function<Traverser, Object> size() {
